...
var Lucl = function(uri, key, displayName, type) {
	...
	this.version = "version 1.4 - limited opening sockets";
	...
	this.message_notstarted += 'Oops, the Weemo Video driver was not detected!';
	...
	switch(this.type) {
		...
		case 'poc':
			this.message_notstarted += '<a href="https://conversations.com-on.com/download/direct-download/domain/55/lang/en" class="button2" id="btnDownload">Download</a>';
		break;
	}
...
Lucl.prototype = {
					...
					switch(event) {
						...
						case "connect":
							...
							switch(this.browser) {
								...
								case "Safari":
								case "Chrome":
									if(this.count == 0) this.initWebsocket();
									this.stateSM = "INIT";
								break;
		...
		switch(this.browser) {
			...
			case 'Explorer':
				...
				if(this.sendroster === true) {
					if(stackPresence != undefined) {
				        if(stackPresence.length > 0 && this.sendpresence === false) {
				        	$.each(stackPresence, function(key,value) {
				        		self.sendMessage(self.myId+":"+value);
	...
	createConf: function(confId) {
		if(this.isConnected) {
	    	...
	    	this.websocket.send('<createcall id="'+id+'" inout="out" stype="'+sessionType+'" uri="999'+confId+'"/>');
	    }
	    else {
